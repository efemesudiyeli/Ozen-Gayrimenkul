...
const navLinks = [
  ...
  { href: "/kibris", label: "Kıbrıs Projeler" },
...
const Header = () => {
  ...
  const navLinkClasses = (transparent: boolean) =>
    `relative font-light transition-colors duration-300 py-2 xl:px-3 ${
      transparent
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20 md:h-auto md:flex-col md:gap-2 xl:flex xl:flex-row xl:items-center xl:justify-between">
            <div className="flex-shrink-0 flex items-center gap-4 md:justify-center md:w-full xl:justify-start xl:w-auto">
              <Image
                src="/logo.png"
                alt="Hatice Özen Gayrimenkul logosu"
                width={50}
                height={50}
                className={
                  logoClasses + "filter transition-[filter] duration-300 invert"
                }
              />
              <Link
                href="/"
                onClick={closeMenu}
                className={logoClasses + " md:text-xl"}
            ...
            <nav
              className="hidden md:flex md:w-full md:justify-center md:flex-nowrap md:items-center md:space-x-4 lg:space-x-8 md:whitespace-nowrap md:overflow-x-auto md:py-2 md:border-t md:border-white/10 xl:w-auto xl:justify-end xl:overflow-visible xl:py-2 xl:border-0"
              aria-label="Ana menü"
            >
              {navLinks.map((link) => (
                <Link
            ...
            <div className="md:hidden xl:justify-self-end">
              ...
              >
                ...
                <div className="w-6 h-6 flex flex-col justify-around">
                  <span
                    className={`block h-0.5 w-full bg-current transform transition duration-300 ease-in-out ${isOpen ? "rotate-45 translate-y-[5px]" : ""}`}
                  ></span>
                  <span
                    className={`block h-0.5 w-full bg-current transition duration-300 ease-in-out ${isOpen ? "opacity-0" : ""}`}
                  ></span>
                  <span
                    className={`block h-0.5 w-full bg-current transform transition duration-300 ease-in-out ${isOpen ? "-rotate-45 -translate-y-[5px]" : ""}`}
                  ></span>
      ...
      <div
        id="mobile-menu"
        className={`fixed top-0 right-0 h-full w-72 bg-white z-50 transform shadow-xl ${isOpen ? "translate-x-0" : "translate-x-full"} transition-transform duration-300 ease-in-out md:hidden`}
      >
        <div className="p-8 flex flex-col h-full">
          <div className="flex-shrink-0 mb-10">
            <Link
              href="/"
              onClick={closeMenu}
              className="text-2xl font-bold text-blue-600 font-roboto"
            >
              Hatice Özen Gayrimenkul
            </Link>
          </div>
