...
const contactQuery = `*[_type == "contactPage"] | order(_updatedAt desc)[0]{
  title,
  heroTitle,
  heroDescription,
  metaDescription,
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contactClient = client.withConfig({ useCdn: false });
    const contactData = await contactClient.fetch<ContactPageData>(
      contactQuery,
      {},
    ...
    return {
      ...
      description:
        ...
        "Bizimle iletişime geçin. Adres, telefon ve e-posta bilgilerimiz.",
    };
  } catch {
    return {
...
const ContactPage = async () => {
  ...
  try {
    const contactClient = client.withConfig({ useCdn: false });
    const fetchedData = await contactClient.fetch<ContactPageData>(
      contactQuery,
      {},
      { next: { revalidate: 0, tags: ["contactPage"] } }
    );
    if (fetchedData) {
  ...
  const localTenFromPrimary = extractLocalTenDigits(
    contactData.contactInfo.phone.number || ""
  );
  const localTenFromDisplay = extractLocalTenDigits(
